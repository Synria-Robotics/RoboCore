...
from robocore.modeling.parser.urdf_parser import URDF
from robocore.modeling.parser.mjcf_parser import MJCF
...
class SDFBase:
    ...
    def sample_sdf_points(self):
        ...
        with warnings.catch_warnings():
            ...
            try:
                if surface_method == 'scan':
                    random_sdf = mesh_to_sdf.mesh_to_sdf(mesh, random_points, surface_point_method='scan')
                else:
                    random_sdf = mesh_to_sdf.mesh_to_sdf(mesh, random_points, surface_point_method='sample')
            except (AttributeError, RuntimeError, OSError) as e:
                # Fallback to 'sample' method if 'scan' fails
                print(f"Warning: 'scan' method failed ({e}), falling back to 'sample' method")
                random_sdf = mesh_to_sdf.mesh_to_sdf(mesh, random_points, surface_point_method='sample')
