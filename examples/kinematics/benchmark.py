...
# -*- coding: utf-8 -*-
"""
Unified Benchmark Suite for RoboCore Kinematics
================================================

This script provides comprehensive benchmarking for:
1. **Performance**: FK/IK/Jacobian speed comparison (NumPy vs PyTorch)
2. **IK Methods**: Comparing pinv, dls, transpose across backends
3. **Parallel Processing**: TRUE batch operations (NumPy vs PyTorch CPU/GPU)

Features:
- Modular subcommands for different benchmark types
- Support for CPU and GPU (CUDA) backends
- Comprehensive statistics (success rate, timing, accuracy)
- Scalability testing with configurable batch sizes

Usage Examples:
--------------
# 1. Quick performance benchmark (FK only)
python benchmark.py performance --fk-runs 1000

# 2. IK methods comparison
python benchmark.py ik-compare --samples 100 --methods pinv dls --backends numpy torch

# 3. Parallel processing benchmark
python benchmark.py parallel --batch-size 1000 --backends numpy torch-cpu torch-cuda
