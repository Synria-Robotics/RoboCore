...
def test_transform_composition(model_path, arm_side='left', verbose=False):
    ...
    # Create a single model with unified configuration space
    model = RobotModel(model_path, base_link=base_link)
    
    if verbose:
        beauty_print(f"{arm_side.upper()} Arm Model (unified config space):", type="info")
        model.summary(show_chain=True)
        beauty_print(f"Total DOF: {model.num_dof}")
    
    # Generate random joint configurations
    num_tests = 10
    np.random.seed(42)
    q_full_batch = model.random_q_batch(num_tests, seed=42, scale=0.8)
    ...
    for i in range(num_tests):
        q_full = q_full_batch[i]
        
        # Compute FK for each segment using dynamic base_link/end_link
        T_base_to_end = model.fk(q_full, base_link=base_link, end_link=end_link, return_end=True)
        T_base_to_intermediate = model.fk(q_full, base_link=base_link, end_link=intermediate_link, return_end=True)
        T_intermediate_to_end = model.fk(q_full, base_link=intermediate_link, end_link=end_link, return_end=True)
...
def test_direct_comparison(model_path, arm_side='left', verbose=False):
    ...
    # Create a single model with unified configuration space
    model = RobotModel(model_path, base_link=base_link_full)
    ...
    for i in range(num_tests):
        q_full_config = q_full_batch[i]
        
        # Method 1: Direct FK from base to end using dynamic base_link/end_link
        T_full = model.fk(q_full_config, base_link=base_link_full, end_link=end_link, return_end=True)
        
        # Method 2: FK from shoulder to end, then compose with base to shoulder
        T_base_to_shoulder = model.fk(q_full_config, base_link=base_link_full, end_link=base_link_shoulder, return_end=True)
        T_shoulder_to_end = model.fk(q_full_config, base_link=base_link_shoulder, end_link=end_link, return_end=True)
