...
from robocore.kinematics.utils import relative_pose_error
...
def cooperative_carry_scenario():
    ...
    fk_zero = bimanual_forward_kinematics(left_model, right_model, q_zero, q_zero, backend='numpy', return_end=True)
    T_left_zero = fk_zero['left']
    T_right_zero = fk_zero['right']
    ...
    fk_grasp = bimanual_forward_kinematics(left_model, right_model, q_grasp_left, q_grasp_right, backend='numpy', return_end=True)
    T_left_grasp = fk_grasp['left']
    T_right_grasp = fk_grasp['right']
    ...
    for i, (desc, offset) in enumerate(waypoint_plan):
        ...
        # Build right arm absolute target from current left and desired relative
        # For initial iteration, use left grasp pose as approximation for leader pose.
        # Then per-iteration, recompute from the newly solved left.
        T_right_target = T_left_target @ T_rel_grasp

        # Solve dual-arm IK using new bimanual interface (independent coordination)
        ik_kwargs = dict(method='dls')
        res_dual = bimanual_inverse_kinematics(
            left_model,
            right_model,
            target_left=T_left_target,
            target_right=T_right_target,
            q0_left=q_current_left,
            q0_right=q_current_right,
            backend='numpy',
            coordination='indep',
            **ik_kwargs,
        )
        ...
        overall_success = bool(
            res_dual.get('success',
                         res_dual.get('success_left', True) and res_dual.get('success_right', True))
        )
        if not overall_success:
            print(f"  {i}. {desc:20s} IK WARNING ⚠️")

        q_current_left = np.array(res_dual.get('q_left', q_current_left))
        q_current_right = np.array(res_dual.get('q_right', q_current_right))
        waypoints_dual.append((q_current_left.copy(), q_current_right.copy()))

        # Validate grasp maintenance
        fk_cur = bimanual_forward_kinematics(left_model, right_model, q_current_left, q_current_right, backend='numpy', return_end=True)
        T_L = fk_cur['left']
        T_R = fk_cur['right']
    ...
    for i in range(0, len(q_left_traj), 50):  # Sample every 50 points
        fk_chk = bimanual_forward_kinematics(left_model, right_model, q_left_traj[i], q_right_traj[i], backend='numpy', return_end=True)
        T_L = fk_chk['left']
        T_R = fk_chk['right']
