...
class BeautyLogger:
    """
    Lightweight logger for RobotCore package.
    """

    def __init__(self, log_dir: str, log_name: str = 'RobotCore.log', verbose: bool = True):
        """
        Lightweight logger for RobotCore package.

        Example::

            >>> from RobotCore.utils.logger import BeautyLogger
            >>> logger = BeautyLogger(log_dir=".", log_name="RobotCore.log", verbose=True)
...
def beauty_print(content, type: str = None, width: int = 80, centered: bool = True):
    ...
    if type is None:
        type = "info"
    if type == "warning":
        print("\033[1;37m[RobotCore:WARNING] {}\033[0m".format(content))  # For warning (gray)
    elif type == "module":
        if centered:
            ...
            print("\033[1;33m[RobotCore:MODULE] {}\033[0m".format(content))  # For a new module (light yellow)
    elif type == "info":
        print("\033[1;35m[RobotCore:INFO] {}\033[0m".format(content))  # For info (light purple)
    elif type == "error":
        print("\033[1;31m[RobotCore:ERROR] {}\033[0m".format(content))  # For error (red)
        raise ValueError(content)
    elif type == "success":
        print("\033[1;32m[RobotCore:SUCCESS] {}\033[0m".format(content))  # For success (green)
    else:
...
def beauty_print_matrix(name: str, data: Any, precision: int = 4, max_batch_items: int = 1, indent: int = 2):
    """
    Pretty print a scalar / vector / matrix / batch of matrices with RobotCore style.
