...
This directory contains example YAML configuration files for RobotCore.
...
python examples/demo_with_config.py --config robotcore/configs/gpu_config.yaml

# Load Bessica robot configuration
python examples/demo_with_config.py --config robotcore/configs/bessica_config.yaml

# Override specific parameters
python examples/demo_with_config.py \
    --config robotcore/configs/default.yaml \
...
```bash
# Copy a template
cp robotcore/configs/default.yaml my_config.yaml
...
```python
from robotcore.configs import ConfigManager

# Load base config
config = ConfigManager('robotcore/configs/default.yaml')
...
See full schema in `robotcore/configs/schemas.py`:
...
python demo_with_config.py --config robotcore/configs/default.yaml
```
...
- `robotcore/configs/schemas.py` - Configuration schemas
- `robotcore/configs/config_manager.py` - ConfigManager API
...
python examples/demo_with_config.py \
    --config robotcore/configs/gpu_config.yaml \
    --random --seed 42

# Example 3: Bessica robot with custom joints
python examples/demo_with_config.py \
    --config robotcore/configs/bessica_config.yaml \
